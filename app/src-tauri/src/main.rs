// clipforge desktop shell. The pipeline is a Python sidecar speaking JSONL
// on stdout (`clipforge --jsonl ...`); this shell spawns it, forwards every
...
fn home_dir() -> PathBuf {
    if let Ok(custom) = std::env::var("CLIPFORGE_HOME") {
        return PathBuf::from(custom);
    }
    dirs_home().join(".clipforge")
}
...
fn pipeline_invocation() -> (String, Vec<String>) {
    if cfg!(debug_assertions) {
        ...
        // bootstraps into CLIPFORGE_HOME on first run (uv handles Python
...
/// One-shot `clipforge ig <args...>` call returning the CLI's JSON line
...
fn export_clip(path: String, title: Option<String>) -> Result<String, String> {
    ...
    let stem = title.unwrap_or_else(|| "clipforge".into());
