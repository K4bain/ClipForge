...
Key resolution: CLIPFORGE_GEMINI_API_KEY env var, then
CLIPFORGE_HOME/secrets.json {"gemini_api_key": "..."} (written by the
...
def gemini_api_key() -> str | None:
    key = os.environ.get("CLIPFORGE_GEMINI_API_KEY")
    if key:
        return key
    ...
    def __init__(self, model: str = GEMINI_MODEL):
        ...
        if not key:
            raise LlmError(
                ...
                "CLIPFORGE_GEMINI_API_KEY), or switch to Ollama mode."
            )
        self._key = key
