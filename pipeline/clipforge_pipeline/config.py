...
Everything lives under CLIPFORGE_HOME (default ~/.clipforge):
...
The desktop app points CLIPFORGE_HOME at its own app-data dir; the CLI uses
...
def home_dir() -> Path:
    return Path(os.environ.get("CLIPFORGE_HOME", str(Path.home() / ".clipforge")))
