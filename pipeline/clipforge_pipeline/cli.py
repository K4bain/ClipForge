...
Tauri shell just spawns `clipforge --jsonl run <source>` and streams.
"""
...
def _stages() -> list[queue.Stage]:
    ...
    # score → camera → render. Stage imports are deferred so `clipforge
...
def cmd_ig(args: argparse.Namespace) -> int:
    ...
    if args.ig_cmd in ("media", "pull") and conn is None:
        print("Not connected. Run: clipforge ig connect --app-id ... --app-secret ...", file=sys.stderr)
        return 2
    if conn is not None:
    ...
    if args.ig_cmd == "pull":
        ...
        if not rows:
            print("No linked clips yet. Post an exported clip, then: clipforge ig link ...")
            return 0
        for row in rows:
