...
Resolution order: CLIPFORGE_FFMPEG env → bundled sidecar binary (packaged
...
# CLIPFORGE_HOME/bin so end users never touch Homebrew.
...
# latest-release URL. ~80 MB once, into CLIPFORGE_HOME/bin.
_STATIC_WINDOWS = (
...
def resolve() -> tuple[str, bool]:
    ...
    bundled = os.environ.get("CLIPFORGE_BUNDLED_FFMPEG")  # set by the app shell
    if bundled:
...
def ensure_capable(progress=None) -> bool:
    ...
    into CLIPFORGE_HOME/bin (macOS arm64/x86_64, Windows x64), then
